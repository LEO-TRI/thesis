...
def load_data():
    ...
    files = glob.glob(folder_path + '/*.csv.gz')
    li = []
...
def text_selector(df:pd.DataFrame):
...
def text_desc_selector(df:pd.DataFrame):
...
def preprocess_data(df):
    ...
    df_merged.loc[:, ["first_review", "last_review"]] = df_merged[["first_review", "last_review"]].apply(pd.to_datetime, axis=0)
    df_merged["time_difference"] = (df_merged["last_review"] - df_merged['first_review']) / np.timedelta64(1, 'D')
    df_merged["nb_of_nights_cons"] = df_merged["number_of_reviews"] / 0.5
    df_merged["nb_of_nights_large"] = df_merged["number_of_reviews"] / 0.72

    df_merged["occupancy_rate_cons"] = df_merged["nb_of_nights_cons"] * df_merged["minimum_nights"]/df_merged["time_difference"]
    df_merged["occupancy_rate_large"] = df_merged["nb_of_nights_large"]* df_merged["minimum_nights"]/df_merged["time_difference"]
    df_merged["occupancy_rate_large"] = np.where(df_merged["occupancy_rate_large"]>100, 100, df_merged["occupancy_rate_large"])
    df_merged["occupancy_rate_cons"] = np.where(df_merged["occupancy_rate_cons"]>100, 100, df_merged["occupancy_rate_cons"])

    df_merged["exit"] = df_merged["last_review"] + np.timedelta64(6, 'M')
    df_merged["entry"] = df_merged["first_review"] - np.timedelta64(6, 'M')

    df_merged.to_parquet('data/df_model.parquet.gzip',
                compression='gzip')
